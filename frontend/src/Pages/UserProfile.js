...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserDetails = async () => {
      try {
        const userDetailsResponse = await fetch(`http://34.224.61.54:8080/Users/username/${username}`);
  ...
  const fetchReviews = async () => {
    if (endFeed) return;
    let endpoint = 'http://34.224.61.54:8080/userName/' + username + '/Reviews';
  ...
  const handleDeleteReview = async (review_id) => {
    ...
    var endpoint = 'http://34.224.61.54:8080/DeleteReview/' + review_id + '/' + localStorage.getItem("user_id");
    try {
      const response = await fetch(endpoint, {
        method: 'DELETE',
      });
    ...
    if (review.syllabusLink) {
      ...
      endpoint = 'http://34.224.61.54:8000/deleteFile/' + file_id;
